...
// Récupérer tous les professeurs créés par cet admin
...
if ($selected_class && $selected_teacher) {
    ...
    $courses = db_fetch_all(
        "SELECT c.*, t.name as teacher_name, cl.name as class_name
         ...
         WHERE c.created_by = ?
         AND (c.classid = ? OR c.classid IS NULL)
         ORDER BY c.name",
        [$admin_id, $selected_class],
        'ss'
    );
}
        ...
        <div class="bg-white rounded-lg shadow p-6 mb-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                <?php foreach ($teachers as $teacher): ?>
                    <a href="?teacher_id=<?php echo htmlspecialchars($teacher['id']); ?><?php echo $selected_class ? '&class_id=' . $selected_class : ''; ?>" 
                       class="block p-4 border rounded-lg hover:bg-gray-50 transition duration-150 ease-in-out <?php echo $selected_teacher === $teacher['id'] ? 'border-blue-500 bg-blue-50' : 'border-gray-200'; ?>">
            ...
            <div class="bg-white rounded-lg shadow p-6 mb-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    <?php foreach ($classes as $class): ?>
                        <a href="?teacher_id=<?php echo htmlspecialchars($selected_teacher); ?>&class_id=<?php echo htmlspecialchars($class['id']); ?>" 
                                    ...
                                    <?php foreach ($courses as $course): ?>
                                        <tr>
                                            ...
                                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                                <?php echo $course['class_name'] ? htmlspecialchars($course['class_name']) : 'Non assigné'; ?>
                                            </td>
                                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                            ...
                                            <td class="px-6 py-4 whitespace-nowrap text-sm">
                                                <form method="POST" class="inline">
                                                    ...
                                                    <input type="hidden" name="teacher_id" value="<?php echo htmlspecialchars($selected_teacher); ?>">
