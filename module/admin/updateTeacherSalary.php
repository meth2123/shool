...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!empty($teacher_id) && !empty($new_salary)) {
        ...
        $check_stmt->bind_param("ss", $teacher_id, $admin_id);
        $check_stmt->execute();
        
        if ($check_stmt->get_result()->num_rows > 0) {
            // Mettre à jour le salaire
            $update_sql = "UPDATE teachers SET salary = ? WHERE id = ? AND created_by = ?";
            $update_stmt = $link->prepare($update_sql);
            $update_stmt->bind_param("dss", $new_salary, $teacher_id, $admin_id);
            ...
            $error_message = "Enseignant non trouvé ou accès non autorisé";
        }
    }
}

// Récupérer la liste des enseignants
$sql = "SELECT id, name, salary FROM teachers WHERE created_by = ? ORDER BY name";
...
<head>
    ...
    <title>Mise à jour des Salaires - Enseignants</title>
...
<body class="bg-gray-100">
    ...
    <div class="container mx-auto px-4 py-8">
        <div class="max-w-4xl mx-auto">
            <h2 class="text-2xl font-bold text-gray-900 mb-6">Mise à jour des Salaires des Enseignants</h2>
                                ...
                                <tr class="hover:bg-gray-50">
                                    <form method="POST" action="" class="contents">
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                            ...
                                            <input type="hidden" name="teacher_id" value="<?php echo htmlspecialchars($row['id']); ?>">
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
