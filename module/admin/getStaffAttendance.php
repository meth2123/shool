...
if (!$staffid) {
    echo json_encode(['error' => 'ID membre du personnel non spécifié']);
    exit();
}

// Vérifier si le membre du personnel appartient à cet administrateur
$check_sql = "SELECT id FROM staff WHERE id = ? AND created_by = ?";
$check_stmt = $link->prepare($check_sql);
$check_stmt->bind_param("ss", $staffid, $admin_id);
$check_stmt->execute();
$check_result = $check_stmt->get_result();

if ($check_result->num_rows === 0) {
    echo json_encode(['error' => 'Membre du personnel non trouvé ou non autorisé']);
    exit();
}
...
$present_stmt->bind_param("s", $staffid);
...
$absent_stmt->bind_param("s", $staffid);
$absent_stmt->execute();
