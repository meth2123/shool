...
CREATE TABLE IF NOT EXISTS `class` (
  ...
  PRIMARY KEY (`id`),
  KEY `created_by` (`created_by`),
  CONSTRAINT `fk_class_admin` FOREIGN KEY (`created_by`) REFERENCES `admin` (`id`) ON DELETE SET NULL ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
CREATE TABLE IF NOT EXISTS `course` (
  ...
  KEY `created_by` (`created_by`),
  CONSTRAINT `course_class_fk` FOREIGN KEY (`class_id`) REFERENCES `class` (`id`) ON DELETE CASCADE,
  CONSTRAINT `course_teacher_fk` FOREIGN KEY (`teacher_id`) REFERENCES `teachers` (`id`) ON DELETE CASCADE,
...
-- Table class déjà définie plus haut
...
INSERT INTO `users` (`userid`, `password`, `usertype`, `status`) VALUES
('0', '123456', 'staff', 'active'),
('ad-123-0', '123', 'admin', 'active'),
('ad-123-1', '$2y$10$ZtVGA.OFGqevA9W7X60s9.2jZnN40Z9McfCwXtxpmpzZryJ5XE5Qm', 'admin', 'active'),
('ad-123-2', '123', 'admin', 'active'),
('ad-123-3', '123', 'admin', 'active'),
('DIO-PA001', 'Alamine', 'parent', 'active'),
('MET-PA001', 'Alamine', 'parent', 'active'),
('MET2813', 'Alamine', 'admin', 'active'),
('PA-123', '123456', 'parent', 'active'),
('pa-123-1', '123', 'parent', 'active'),
('pa-124-1', '123', 'parent', 'active'),
('st-123-1', '123', 'student', 'active'),
('st-124-1', '125', 'student', 'active'),
('st-125-1', '123', 'student', 'active'),
-- Nouveaux utilisateurs pour les tests
('admin', '$2y$10$YourHashedPasswordHere', 'admin', 'active'),
('teacher1', '$2y$10$YourHashedPasswordHere', 'teacher', 'active'),
('teacher2', '$2y$10$YourHashedPasswordHere', 'teacher', 'active'),
('student1', '$2y$10$YourHashedPasswordHere', 'student', 'active'),
('student2', '$2y$10$YourHashedPasswordHere', 'student', 'active'),
('parent1', '$2y$10$YourHashedPasswordHere', 'parent', 'active'),
('parent2', '$2y$10$YourHashedPasswordHere', 'parent', 'active'),
('staff1', '$2y$10$YourHashedPasswordHere', 'staff', 'active'),
-- Utilisateur avec statut inactif pour tester la fonctionnalité d'abonnement expiré
('expired_user', '$2y$10$YourHashedPasswordHere', 'admin', 'inactive'),
('st001', 'bonjour', 'student', 'active'),
('st0112', '$2y$10$CDrSJTSYX7Keu5EU0m00U.CMHe9eS2ByUEyuRPHSfmVHIvWRBGl2y', 'student', 'active'),
('st1', '$2y$10$uAm7SXDIsBh4dplQJ4DHa.s0YTLtu/lZ8xHyaAdG/hvWJpL8iYZai', 'student', 'active'),
('st200', '$2y$10$x3pXRf3TEMwXtrm14mVL7uZKMFLFWykWx4Nvw9ODDkx9uRXQwgdei', 'student', 'active'),
('sta-123-1', 'doule123', 'staff', 'active'),
('sta-124-1', 'doule123', 'staff', 'active'),
('sta-125-1', 'doule123', 'staff', 'active'),
('sta-126-1', 'doule123', 'staff', 'active'),
('sta200', '$2y$10$lwyDr9DtIW/2lW6mNIMBVOy5oWd5Y2kLKGmfWkJiKhgHawJ.QL6Re', 'student', 'active'),
('STF001ITA', 'doule123', 'staff', 'active'),
('te-123-1', '123', 'teacher', 'active'),
('te-124-1', '124', 'teacher', 'active'),
('te-125-1', '258', 'teacher', 'active'),
('te-126-1', '258', 'teacher', 'active'),
('te-127-1', '123', 'teacher', 'active'),
('TE-ABD-4064', '$2y$10$Gy8eJAyoSEG9sKC8vJRn7e2RF8mB49OQ/.xqNTZa8ouw476H/m69i', 'teacher', 'active'),
('TE-MET-1874', '$2y$10$xJIhmJdIyt7g4bEecl97k.678msrDnNzAzzerRYF22f0Y9BcpEC9e', 'teacher', 'active'),
('TE-MET-2090', '$2y$10$0Hwm/oK5H3uDz8KOMdankeWsrH3BRKPzlicFN70Dw3M.BUjE95L36', 'teacher', 'active'),
('TE-MET-8005', 'meth21', 'teacher', 'active');
...
CREATE TABLE IF NOT EXISTS `subscriptions` (
  ...
  PRIMARY KEY (`id`),
  KEY `user_id` (`user_id`),
  CONSTRAINT `subscriptions_user_fk` FOREIGN KEY (`user_id`) REFERENCES `users` (`userid`) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
SET FOREIGN_KEY_CHECKS=0;
-- Contraintes de clé étrangère désactivées pour résoudre les problèmes d'initialisation
-- ALTER TABLE `payment`
...
-- Contraintes pour la table `class` (désactivées pour éviter les duplications)
-- ALTER TABLE `class`
...
-- Contraintes pour la table `class_payment_amount` (désactivées pour éviter les problèmes d'initialisation)
-- ALTER TABLE `class_payment_amount`
--   ADD CONSTRAINT `class_payment_amount_ibfk_1` FOREIGN KEY (`class_id`) REFERENCES `class` (`id`) ON DELETE CASCADE;

-- Contraintes pour la table `exams` (désactivées pour éviter les problèmes d'initialisation)
-- ALTER TABLE `exams`
...
-- Contraintes pour la table `exam_results` (désactivées pour éviter les problèmes d'initialisation)
...
-- Contraintes pour la table `parents` (désactivées pour éviter les problèmes d'initialisation)
-- ALTER TABLE `parents`
--   ADD CONSTRAINT `parents_ibfk_1` FOREIGN KEY (`created_by`) REFERENCES `admin` (`id`);

-- Contraintes pour la table `staff` (désactivées pour éviter les problèmes d'initialisation)
-- ALTER TABLE `staff`
--   ADD CONSTRAINT `staff_ibfk_1` FOREIGN KEY (`created_by`) REFERENCES `admin` (`id`);

-- Contraintes pour la table `staff_salary_history` (désactivées pour éviter les problèmes d'initialisation)
...
-- Contraintes pour la table `student_teacher_course` (désactivées pour éviter les problèmes d'initialisation)
...
-- Contraintes pour la table `subjects` (désactivées pour éviter les problèmes d'initialisation)
-- ALTER TABLE `subjects`
--   ADD CONSTRAINT `subjects_ibfk_1` FOREIGN KEY (`created_by`) REFERENCES `admin` (`id`) ON DELETE CASCADE;

-- Contraintes pour la table `teachers` (désactivées pour éviter les problèmes d'initialisation)
-- ALTER TABLE `teachers`
--   ADD CONSTRAINT `teachers_ibfk_1` FOREIGN KEY (`created_by`) REFERENCES `admin` (`id`);

-- Contraintes pour la table `teacher_salary_history` (désactivées pour éviter les problèmes d'initialisation)
